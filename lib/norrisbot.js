...
class NorrisBot extends Bot {
    constructor(settings) {
        ...
        this.settings.name = this.settings.name || 'chuck';
        this.dbJokesPath = settings.dbPath || path.resolve(process.cwd(), 'data', 'chucknorris_jokes.db');
...
NorrisBot.prototype._connectDb = function () {
    if (!fs.existsSync(this.dbJokesPath)) {
        console.error('Database path ' + '"' + this.dbJokesPath + '" does not exists or it\'s not readable.');
        process.exit(1);
    }

    this.db = new SQLite.Database(this.dbJokesPath);
};
